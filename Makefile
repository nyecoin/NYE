...
pkgdatadir = $(datadir)/nyecoin
pkgincludedir = $(includedir)/nyecoin
pkglibdir = $(libdir)/nyecoin
pkglibexecdir = $(libexecdir)/nyecoin
...
CONFIG_HEADER = $(top_builddir)/src/config/nyecoin-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/nyecoin-config.h.in \
...
ACLOCAL = ${SHELL} /home/nyecoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/nyecoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/nyecoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/nyecoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = nyecoin-cli
BITCOIN_DAEMON_NAME = nyecoind
BITCOIN_GUI_NAME = nyecoin-qt
BITCOIN_TX_NAME = nyecoin-tx
...
MAINT = 
MAKEINFO = ${SHELL} /home/nyecoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
PACKAGE_NAME = NYECOIN
PACKAGE_STRING = NYECOIN 2.0.0
PACKAGE_TARNAME = nyecoin
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/nyecoin
abs_srcdir = /home/nyecoin
abs_top_builddir = /home/nyecoin
abs_top_srcdir = /home/nyecoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/nyecoin/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_nyecoin_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_nyecoin_coverage.info test_nyecoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/NYEcoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/nyecoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/nyecoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/nyecoin-config.h
$(top_srcdir)/src/config/nyecoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/nyecoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/NYEcoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/NYEcoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/NYEcoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_nyecoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_nyecoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_nyecoin_filtered.info: test_nyecoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_nyecoin_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_nyecoin_coverage.info: baseline_filtered_combined.info test_nyecoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_nyecoin_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_nyecoin_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_nyecoin_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_nyecoin.coverage/.dirstamp:  test_nyecoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_nyecoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_nyecoin.coverage/ total.coverage/ $(OSX_APP)
